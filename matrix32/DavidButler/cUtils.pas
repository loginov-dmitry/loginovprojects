...
{                     comparison functions. Added support for Currency type.   }
{                                                                              }
interface
...
const
  ...
  UnitCopyright = '(c) 2000-2003 David J Butler';
...
const
  MinByte     : Byte     = Low(Byte);
  MaxByte     : Byte     = High(Byte);
  MinWord     : Word     = Low(Word);
  MaxWord     : Word     = High(Word);
  MinLongWord : LongWord = Low(LongWord);
  MaxLongWord : LongWord = High(LongWord);
  MinShortInt : ShortInt = Low(ShortInt);
  MaxShortInt : ShortInt = High(ShortInt);
  MinSmallInt : SmallInt = Low(SmallInt);
  MaxSmallInt : SmallInt = High(SmallInt);
...
{   HashStr will sample up to 1/16th of the string for large strings.          }
{                                                                              }
...
{   'mod' operator used when Slots <> 0 is comparitively slow.                 }  
{                                                                              }
...
begin
  if Ch in C then
    Exclude(C, AnsiChar(Ch)) else
    Include(C, AnsiChar(Ch));
end;
{$ENDIF}
    ...
    if Ch in DestSet then
      begin
        if Ch in SourceSet then
          Exclude(DestSet, AnsiChar(Ch));
      end else
      if Ch in SourceSet then
        Include(DestSet, AnsiChar(Ch));
end;
{$ENDIF}
...
begin
  For Ch := 'A' to 'Z' do
    if Ch in C then
      Include(C, AnsiChar(Char(Ord(Ch) + 32)));
  For Ch := 'a' to 'z' do
    if Ch in C then
      Include(C, AnsiChar(Char(Ord(Ch) - 32)));
end;
